...
async function loadTransformers(){
  ...
  // Browser-specific bundle avoids loading a Node-oriented runtime in managed Chrome/Chromebook environments.
  ...
  if(env.backends?.onnx?.wasm){
    env.backends.onnx.wasm.wasmPaths={
      mjs:self.location.origin+'/api/ai/assets/ort-wasm-simd-threaded.mjs',
      wasm:self.location.origin+'/api/ai/assets/ort-wasm-simd-threaded.wasm'
    };
...
self.onmessage=async(e)=>{
  ...
  try{
    ...
    const tokenBudget = jsonTask ? 260 : (writingTask ? 300 : (complex ? 200 : 150));
    const base={
