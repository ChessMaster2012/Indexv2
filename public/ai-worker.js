...
async function loadGeneratorUnlocked(){
  ...
  // RESTORED STABLE TUTOR:
  // Use the small Qwen2.5 0.5B model only. Do not use q4f16/FP16 weights or the
  // larger 1.5B model because those can trigger the WebGPU Cast float16/float32
  // graph error seen on some Chrome/Chromebook configurations.
  if(gpuInfo.ok){
    try{
      status('Loading the Tutor…');
      ...
      status('Tutor ready','ready');
      return generator;
    }catch(e){
      console.warn('Stable WebGPU tutor unavailable; falling back to CPU/WASM.',e);
      generator=null;
    }
  }
