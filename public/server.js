...
function extractAiText(data){
  ...
  for(const value of directCandidates){
    ...
    if(Array.isArray(value)){
      const joined = value.map(x => typeof x === 'string' ? x : x?.text || '').join('').trim();
      if(joined) return joined;
    }
  }
  if(Array.isArray(data?.output)){
    const joined = data.output.map(x => typeof x === 'string' ? x : x?.content || x?.text || '').join('').trim();
    if(joined) return joined;
  }
  return '';
}
...
async function fetchProvider(provider, messages){
  ...
  try{
    ...
    if(data?.error){
      const err = new Error(`${provider.name}: ${extractProviderError(data) || 'provider error'}`);
      err.status = Number(data?.error?.status || 0) || undefined;
      throw err;
    }
