...
async function loadTransformers(){
  ...
  // Transformers.js 3.8.1 is the stable browser build and keeps the runtime self-contained for this deployment. It includes
...
async function loadGenerator(){
  ...
  if(gpuOk){
    try{
      ...
      generator=await pipeline('text-generation',WEBGPU_MODEL,{device:'webgpu',dtype:'q4f16',max_new_tokens:192});
  ...
  generator=await pipeline('text-generation',WASM_MODEL,{device:'wasm',dtype:'q8',max_new_tokens:160});
...
self.onmessage=async(e)=>{
  ...
  try{
    ...
    const base={
      // Keep normal answers short so the local model can usually finish quickly.
      max_new_tokens:jsonTask?300:(thinking?260:160),
      do_sample:true,
