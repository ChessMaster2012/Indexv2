const LOCAL_MODEL='onnx-community/Qwen3-0.6B-Instruct-ONNX';
const WEBGPU_MODEL=LOCAL_MODEL;
const WASM_MODEL=LOCAL_MODEL;
...
async function loadGeneratorUnlocked(){
  ...
  // Stable/known-good local tutor path: Qwen3 0.6B.
  // Use the same small model on every device; only the runtime backend/quantization changes.
  // This avoids trying a larger model first and keeps startup predictable.
  if(gpuInfo.ok){
    if(gpuInfo.f16){
      try{
        status('Hardware acceleration detected • loading the compatible tutor…');
        generator=await pipeline('text-generation',WEBGPU_MODEL,{...common,device:'webgpu',dtype:'q4f16',max_new_tokens:260});
        deviceMode='WebGPU Qwen3 0.6B';
        status('Local AI ready • compatible GPU tutor','ready');
        return generator;
      }catch(e){
        console.warn('Qwen3 0.6B WebGPU q4f16 unavailable; retrying with q4.',e);
        generator=null;
        status('Trying the compatible GPU mode…');
      }
    }

    try{
      generator=await pipeline('text-generation',WEBGPU_MODEL,{...common,device:'webgpu',dtype:'q4',max_new_tokens:240});
      deviceMode='WebGPU Qwen3 0.6B';
      status('Local AI ready • compatible GPU tutor','ready');
      return generator;
    }catch(e){
      console.warn('Qwen3 0.6B WebGPU q4 unavailable; using WASM/CPU.',e);
      generator=null;
      status('GPU mode unavailable • switching to compatible CPU mode…');
    }
  }else{
  ...
  generator=await pipeline('text-generation',WASM_MODEL,{...common,device:'wasm',dtype:'q4',max_new_tokens:200});
  deviceMode='WASM Qwen3 0.6B';
  status('Local AI ready • compatible CPU tutor','ready');
  return generator;
}
