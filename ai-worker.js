...
async function loadGeneratorUnlocked(){
  ...
  if(gpuInfo.ok){
    ...
    if(gpuInfo.f16){
      try{
        ...
        generator=await pipeline('text-generation',WEBGPU_STRONG_MODEL,{...common,device:'webgpu',dtype:'q4f16',max_new_tokens:640});
    ...
    try{
      generator=await pipeline('text-generation',WEBGPU_FALLBACK_MODEL,{...common,device:'webgpu',dtype:'q4',max_new_tokens:420});
  ...
  generator=await pipeline('text-generation',WASM_MODEL,{...common,device:'wasm',dtype:'q4',max_new_tokens:360});
