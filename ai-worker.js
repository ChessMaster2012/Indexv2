const WEBGPU_STRONG_MODEL='onnx-community/Qwen2.5-1.5B-Instruct';
const WEBGPU_FALLBACK_MODEL='onnx-community/Qwen2.5-0.5B-Instruct';
const WASM_MODEL='onnx-community/Qwen2.5-0.5B-Instruct';
let transformers=null, generator=null, generatorPromise=null, deviceMode='';
let generationBusy=false;
// The model is initialized once, before the student submits a question. Never load a model during generation.
...
async function getWebGPUInfo(){
  if(!navigator.gpu || typeof navigator.gpu.requestAdapter!=='function') return {ok:false,f16:false};
  try{
    const adapter=await navigator.gpu.requestAdapter({powerPreference:'high-performance'});
    if(!adapter) return {ok:false,f16:false};
    const features=adapter.features;
    return {ok:true,f16:!!(features && typeof features.has==='function' && features.has('shader-f16'))};
  }catch(e){ return {ok:false,f16:false}; }
}
...
async function loadGeneratorUnlocked(){
  ...
  if(gpuInfo.ok){
    // Strong desktop/GPU path: Qwen2.5 1.5B Instruct. It is substantially larger than
    // the old 0.6B model and is intended for better writing, explanation, and multi-step work.
    // Only use the FP16-weight quantization when the adapter reports shader-f16.
    if(gpuInfo.f16){
      try{
        status('Hardware acceleration detected • loading the strong tutor…');
        generator=await pipeline('text-generation',WEBGPU_STRONG_MODEL,{...common,device:'webgpu',dtype:'q4f16',max_new_tokens:320});
        deviceMode='WebGPU Qwen2.5 1.5B';
        status('Local AI ready • strong GPU tutor','ready');
        return generator;
      }catch(e){
        console.warn('Strong WebGPU model unavailable; falling back to smaller GPU model.',e);
        generator=null;
        status('Switching to the compatible GPU tutor…');
      }
    }else{
      status('GPU detected without shader-f16 • loading the compatible GPU tutor…');
    }

    // Compatibility GPU fallback: Qwen2.5 0.5B q4 is a documented Transformers.js path
    // and avoids the Qwen3 FP16 Cast graph that repeatedly failed in this app.
    try{
      generator=await pipeline('text-generation',WEBGPU_FALLBACK_MODEL,{...common,device:'webgpu',dtype:'q4',max_new_tokens:240});
      deviceMode='WebGPU Qwen2.5 0.5B';
      status('Local AI ready • GPU compatibility mode','ready');
      return generator;
    }catch(e){
      console.warn('WebGPU fallback unavailable; using WASM/CPU.',e);
      generator=null;
      status('GPU unavailable • switching to compatible CPU mode…');
    }
  }else{
    status('No WebGPU on this device • using the compatible CPU tutor…');
  }

  // CPU/Chromebook path: Qwen2.5 0.5B Instruct q4. This model is specifically documented
  // for Transformers.js browser use and is much safer for memory-constrained WASM devices.
  status('Loading the compatible CPU tutor…');
  generator=await pipeline('text-generation',WASM_MODEL,{...common,device:'wasm',dtype:'q4',max_new_tokens:220});
  deviceMode='WASM Qwen2.5 0.5B';
  status('Local AI ready • CPU/WASM tutor','ready');
  return generator;
}
async function loadGenerator(){
  ...
  generatorPromise=(async()=>{
    try{
      return await loadGeneratorUnlocked();
    }finally{
      // Keep the resolved generator; clear only a failed/incomplete load so a later
      // request can retry instead of spawning multiple pipelines at once.
      if(!generator) generatorPromise=null;
    }
  })();
...
function normalizeOutput(input){
  ...
  // Some instruction models can emit hidden reasoning blocks. Index only displays
...
self.onmessage=async(e)=>{
  ...
  try{
    ...
    const gpuDevice=/^WebGPU/.test(deviceMode);
    const tokenBudget = jsonTask ? (gpuDevice ? 300 : 190) : (writingTask ? (gpuDevice ? 300 : 190) : (complex ? (gpuDevice ? 240 : 170) : (gpuDevice ? 180 : 130)));
    const base={
