...
async function loadGenerator(){
  ...
  if(gpuInfo.ok){
    // q4f16 is fastest when the Chromebook exposes shader-f16. Otherwise use q4,
    // which keeps FP32 activations and avoids the FP16 Cast type mismatch seen on
    // some browser/ONNX Runtime combinations.
    const preferredGpuDtype=gpuInfo.f16 ? 'q4f16' : 'q4';
    try{
      status(gpuInfo.f16 ? 'Using hardware acceleration • loading fast local tutor…' : 'GPU detected • loading compatible local tutor…');
      generator=await pipeline('text-generation',WEBGPU_MODEL,{...common,device:'webgpu',dtype:preferredGpuDtype,max_new_tokens:320});
      deviceMode=gpuInfo.f16 ? 'WebGPU q4f16' : 'WebGPU q4';
      status(`Local AI ready • ${deviceMode}`,'ready');
      return generator;
    }catch(e){
      console.warn(`WebGPU ${preferredGpuDtype} unavailable; retrying WebGPU q4.`,e);
      generator=null;
      try{
        status('GPU compatibility mode • loading q4…');
        generator=await pipeline('text-generation',WEBGPU_MODEL,{...common,device:'webgpu',dtype:'q4',max_new_tokens:320});
        deviceMode='WebGPU q4';
  ...
  // Important: use q4 on WASM rather than q8. The Qwen3 ONNX q4 export is the
  // compatible low-memory CPU path; q8 can select a mixed-precision graph that
  // triggers ONNX Runtime Cast type errors on some Chromebooks.
  status('Loading the compatible local CPU tutor…');
  generator=await pipeline('text-generation',WASM_MODEL,{...common,device:'wasm',dtype:'q4',max_new_tokens:320});
  deviceMode='WASM q4';
  status('Local AI ready • CPU/WASM q4','ready');
  return generator;
}
...
self.onmessage=async(e)=>{
  ...
  try{
    ...
    const tokenBudget = jsonTask ? 280 : (writingTask ? (gpuDevice ? 520 : 300) : (complex ? (gpuDevice ? 300 : 190) : (gpuDevice ? 210 : 140)));
    const base={
