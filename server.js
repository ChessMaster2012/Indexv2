...
// Proxy the browser AI runtime and model assets through this app. This keeps
// managed school browsers from needing direct access to public CDNs/HuggingFace.
app.get('/api/ai/transformers', async (req, res) => {
  try {
    const upstream = await fetch('https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1/+esm');
    if (!upstream.ok) throw new Error(`Transformers CDN returned ${upstream.status}`);
    let source = await upstream.text();
    // Rewrite every CDN/model reference that the browser might otherwise
    // request directly. jsDelivr's +esm output can contain absolute URLs,
    // /npm/... paths, or quoted import strings depending on the version.
    source = source
      .replaceAll('https://cdn.jsdelivr.net/', '/api/ai/cdn/')
      .replaceAll('http://cdn.jsdelivr.net/', '/api/ai/cdn/')
      .replaceAll('https://huggingface.co/', '/api/ai/hf/')
      .replaceAll('http://huggingface.co/', '/api/ai/hf/')
      .replaceAll('https://hf.co/', '/api/ai/hf/')
      .replaceAll('http://hf.co/', '/api/ai/hf/')
      .replaceAll('from \"/npm/', 'from \"/api/ai/cdn/npm/')
      .replaceAll("from '/npm/", "from '/api/ai/cdn/npm/")
      .replaceAll('from \"https://cdn.jsdelivr.net/', 'from \"/api/ai/cdn/')
      .replaceAll("from 'https://cdn.jsdelivr.net/", "from '/api/ai/cdn/")
      .replaceAll('from \"https://huggingface.co/', 'from \"/api/ai/hf/')
      .replaceAll("from 'https://huggingface.co/", "from '/api/ai/hf/");
    res.set('cache-control', 'public, max-age=3600');
    res.type('application/javascript').send(source);
  } catch (err) {
    console.error('AI runtime proxy failed:', err);
    res.status(502).json({ error: 'AI runtime unavailable.' });
  }
});

app.get('/api/ai/cdn/*', async (req, res) => {
  try {
    ...
    console.error('AI CDN proxy failed:', err);
    res.status(502).end();
  }
});

app.get('/api/ai/hf/*', async (req, res) => {
  try {
    const target = 'https://huggingface.co/' + req.params[0];
    const headers = {};
    ...
    console.error('AI model proxy failed:', err);
    res.status(502).end();
  }
});

// Server-side AI fallback. The browser only talks to this same-origin endpoint,
// so school-managed browsers do not need to load the model or AI runtime directly.
let serverAiPipelinePromise = null;
async function getServerAiPipeline(){
  if(!serverAiPipelinePromise){
    serverAiPipelinePromise = import('@huggingface/transformers').then(async ({ pipeline, env }) => {
      env.allowRemoteModels = true;
      env.allowLocalModels = false;
      env.useFSCache = true;
      return pipeline('text-generation', 'onnx-community/SmolLM-135M-Instruct-ONNX', {
        dtype: 'q4'
      });
    }).catch(err => {
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    if(Array.isArray(generated)) {
      ...
      content = typeof last === 'string' ? last : String(last?.content || '');
    } else if(typeof generated === 'string') {
      content = generated;
    }
    if(!content.trim()) throw new Error('AI returned an empty result.');
    res.json({ content });
  } catch(err) {
    console.error('Server AI failed:', err);
    res.status(502).json({ error: 'Server AI unavailable: ' + (err?.message || 'unknown error') });
  }
});
