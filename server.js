...
// School-network-friendly tutor endpoint.
// This endpoint intentionally has NO browser WebGPU/model download and NO
// dependency on an anonymous third-party AI service. That makes it usable on
// managed MCPS Chromebooks where external model downloads or legacy AI queues
// may be blocked/unavailable.
function lastStudentMessage(messages) {
...
function offlineTutorAnswer(question) {
  ...
  if (!q) return 'Tell me what you are studying, and I will help you work through it.';

  // Common math: basic arithmetic.
  ...
  if (arithmetic) {
    ...
    if (result !== null) return `### Let's solve it\n\n${a} ${op} ${b} = **${result}**\n\nIf you want, send the full problem and I can explain each step.`;
  }

  // Common linear equation forms: ax + b = c.
  const eq = q.match(/^\s*(-?\d+(?:\.\d+)?)?\s*x\s*([+\-])\s*(\d+(?:\.\d+)?)\s*=\s*(-?\d+(?:\.\d+)?)\s*\??\s*$/i);
  if (eq) {
    const a = Number(eq[1] || 1), sign = eq[2], b0 = Number(eq[3]), c = Number(eq[4]);
    const b = sign === '-' ? -b0 : b0;
    if (a !== 0) {
      const x = (c - b) / a;
      return `### Solve for x\n\n1. Start with ${a}x ${b >= 0 ? '+' : '-'} ${Math.abs(b)} = ${c}.\n2. Move the constant: ${a}x = ${c - b}.\n3. Divide by ${a}: **x = ${x}**.`;
    }
  }

  const percent = q.match(/(?:what is|calculate)\s+(\d+(?:\.\d+)?)\s*%\s*(?:of)\s*(\d+(?:\.\d+)?)/i);
  if (percent) {
    const p = Number(percent[1]), n = Number(percent[2]);
    return `### Percent\n\n${p}% of ${n} = (${p} ÷ 100) × ${n} = **${(p / 100) * n}**.`;
  }

  const concepts = [
    [/\bfaction\b/, 'A faction is a group of people who share a political interest or goal. In government, factions can organize to influence policy.'],
    [/\bchecks and balances\b/, 'Checks and balances means each branch of the U.S. government has powers that can limit the other branches, helping prevent one branch from becoming too powerful.'],
    [/\bseparation of powers\b/, 'Separation of powers divides federal government responsibilities among the legislative, executive, and judicial branches.'],
    [/\bpopular sovereignty\b/, 'Popular sovereignty is the idea that government gets its authority from the people.'],
    [/\blimited government\b/, 'Limited government means government officials and institutions must follow established laws and constitutional limits.'],
    [/\bnatural rights\b/, 'Natural rights are rights people are understood to have simply because they are human. John Locke commonly described life, liberty, and property as natural rights.'],
    [/\bsocial contract\b/, 'Social contract theory says people accept government authority in exchange for protection of rights and order.'],
    [/\bjudicial review\b/, 'Judicial review is the power of courts to determine whether government actions or laws are consistent with the Constitution. In U.S. history, Marbury v. Madison (1803) established the principle of judicial review.'],
    [/\bcommerce clause\b/, 'The Commerce Clause gives Congress power to regulate commerce among the states, with foreign nations, and with Native American tribes.'],
    [/\b10th amendment\b/, 'The Tenth Amendment says powers not delegated to the United States by the Constitution, and not prohibited to the states, are reserved to the states or the people.'],
    [/\bfederalist 51\b/, 'Federalist No. 51 argues that government should be structured so that ambition counteracts ambition. Its central idea is that separation of powers and checks and balances can help control the concentration of power.'],
    [/\bfederalist 10\b/, 'Federalist No. 10 discusses factions. Madison argues that a large republic can make it harder for a single faction to dominate because many different interests exist.'],
    [/\bshays.? rebellion\b/, 'Shays’ Rebellion was a 1786–1787 uprising by Massachusetts farmers facing economic problems and debt. It increased concern that the Articles of Confederation created a national government that was too weak.'],
    [/\bphotosynthesis\b/, 'Photosynthesis is the process plants and some other organisms use to convert light energy into chemical energy. In simplified form: carbon dioxide + water + light energy → glucose + oxygen.'],
    [/\bmitochondria\b|\bmitochondrion\b/, 'Mitochondria are organelles that carry out much of a cell’s aerobic cellular respiration and produce ATP, the cell’s main readily usable energy currency.'],
    [/\bdna\b/, 'DNA is the molecule that stores hereditary genetic information. Its sequence provides instructions used by cells to make proteins and regulate biological processes.'],
    [/\bgravity\b/, 'Gravity is an attractive interaction between objects with mass. Near Earth’s surface, it causes objects to accelerate downward at about 9.8 m/s² when air resistance is ignored.'],
    [/\bnewton.?s first law\b/, 'Newton’s First Law says an object remains at rest or moves at constant velocity unless acted on by a net external force.'],
    [/\bnewton.?s second law\b/, 'Newton’s Second Law relates net force, mass, and acceleration: F = m × a.'],
    [/\bnewton.?s third law\b/, 'Newton’s Third Law says forces come in equal-and-opposite pairs acting on different objects.'],
    [/\bmetaphor\b/, 'A metaphor compares two unlike things by saying one thing is another, helping communicate an idea or quality without using “like” or “as.”'],
    [/\birony\b/, 'Irony involves a contrast between expectation and what actually happens or what is meant. Dramatic irony occurs when the audience knows something important that a character does not.'],
    [/\bconstitution\b/, 'The U.S. Constitution establishes the structure and powers of the federal government and places limits on those powers.'],
    [/\bbill of rights\b/, 'The Bill of Rights is the first ten amendments to the U.S. Constitution. It protects individual liberties and places limits on federal government power.']
  ];
  for (const [re, answer] of concepts) if (re.test(l)) return `### Study help\n\n${answer}\n\n**Quick check:** Explain the idea in your own words, or send the exact question you were given.`;

  if (/\b(spanish|español|hola|verb|conjugat|vocab)/i.test(q)) {
    return '### Spanish help\n\nI can help you practice vocabulary, grammar, and conjugation. Send the Spanish word, sentence, or exact exercise and I’ll explain the rule and work through an example with you.';
  }
  if (/\b(essay|thesis|paragraph|english class|literary)/i.test(q)) {
    return '### Writing help\n\nStart with your main claim, then give specific evidence and explain how that evidence supports the claim. Send your prompt or paragraph and I can help you organize or improve the reasoning.';
  }

  return `### Tutor help\n\nI’m running in **MCPS-compatible study mode**, so this page does not need WebGPU, a downloaded browser model, or an external AI account.\n\nI can still help with common school topics, math steps, and study concepts. For the most useful help, send the exact problem, question, or notes you are working from.\n\n**Tip:** Include your class/subject (for example, “Geometry” or “AP Government”) and I’ll tailor the explanation.`;
}
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    // Optional provider hook: if a future school-approved provider is configured,
    // it can be added here without changing the browser client. For the current
    // deployment, the built-in tutor is the reliable no-key fallback.
    const answer = offlineTutorAnswer(lastStudentMessage(messages));
    res.json({ content: answer, mode: 'mcps-compatible' });
  } catch (err) {
    console.error('Tutor failed:', err);
    res.status(500).json({ error: 'The tutor could not process that request. Please try again.' });
  }
});
