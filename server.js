...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    const tutorQuality = `You are a high-quality, careful school tutor. Give correct, useful answers rather than generic filler.
...
io.on('connection', socket => {
  ...
  socket.on('player:join', (payload, cb) => {
    ...
    if (existing) {
      ...
      room.players.set(id, { id, name: (name || 'Player').slice(0, 40), score: 0, correctCount: 0, gold: 0, chestsAvailable: 0, alive: true, streak: 0, online: true, socketId: socket.id });
    }
