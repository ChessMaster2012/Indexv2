...
app.get('/api/ai-status', (req, res) => { res.json({ enabled: true }); });
...
app.post('/api/auth/signup', (req, res) => {
  ...
  const user = { id: crypto.randomBytes(12).toString('hex'), method:'email', identifier, username, passwordHash: hash, salt, firstName:'', lastName:'', createdAt: Date.now(), accountData: null };
...
app.put('/api/account/state', (req,res)=>{
  if(!req.user) return res.status(401).json({error:'Not signed in.'});
  req.user.accountData = sanitizeAccountState(req.body?.state);
  saveUsers();
  res.json({ok:true, state:req.user.accountData});
});
